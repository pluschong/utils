 ...
 */
export function deepGet(obj: SafeAny, path: string | string[] | null | undefined, defaultValue?: unknown): SafeAny {
 ...
 */
export function deepCopy<T extends { [key: string]: SafeAny } = SafeAny>(obj: T | null | undefined): T {
 ...
 */
export function deepMergeKey(original: unknown, arrayProcessMethod: boolean, ...objects: SafeAny[]): SafeAny {
	if (Array.isArray(original) || typeof original !== 'object') return original;

	const isObject = (v: unknown): boolean => typeof v === 'object';

	const merge = (target: SafeAny, obj: SafeAny): SafeAny => {
		Object.keys(obj)
 ...
 * 深度合并对象
 */
export function deepMerge(original: unknown, ...objects: unknown[]): SafeAny {
